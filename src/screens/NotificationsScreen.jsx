    ...
    <div className="fixed inset-0 z-50 bg-stone-100">
      <div className="max-w-lg mx-auto h-full flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 bg-white border-b border-stone-200">
          <h1 className="font-display text-lg font-extrabold text-stone-900">Notifications</h1>
          <button onClick={onClose} className="text-stone-400"><X size={22} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {alerts.length === 0 && (
            <p className="text-center text-stone-400 mt-10">No notifications yet</p>
          )}
          {alerts.map((a) => {
              ...
              <div key={a.id} className="bg-white rounded-2xl border border-stone-200 p-3.5 flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-extrabold text-[13.5px] text-stone-900 truncate">{a.title}</span>
                    <span className="text-[11px] text-stone-400 whitespace-nowrap">{timeAgo(a.time)}</span>
                  </div>
                  <p className="text-[12.5px] text-stone-500 mt-0.5">{a.message}</p>
                </div>
              </div>
            );
          })}
